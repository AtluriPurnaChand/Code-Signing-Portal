...
app.secret_key = os.environ.get('FLASK_SECRET', 'mysecret123')  # consider setting via env var
...
# Fix: use __file__ to compute paths
...
# set MAIL_USERNAME and MAIL_PASSWORD as environment variables in production
<<<<<<< HEAD
app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME', "codesigningportal@gmail.com")
app.config['MAIL_PASSWORD'] = os.environ.get('MAIL_PASSWORD', "replace_with_real_password")
=======
app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME', "codesigningportal1@gmail.com")
app.config['MAIL_PASSWORD'] = os.environ.get('MAIL_PASSWORD', "eaec kzgn vbxw ndjo")
>>>>>>> 013002583c9c6944a86a45dd2040e9c466978da7
...
# --- Ensure unique email ---
try:
    users.create_index([("email", 1)], unique=True, sparse=True, name="unique_email")
    print("✅ Unique index on email ensured")
except Exception as e:
    if "E11000" in str(e) or "already exists" in str(e):
        print("ℹ Unique index on email already exists, skipping.")
    else:
...
QUARANTINE_DIR = os.path.join(APP_DIR, 'tmp', 'quarantine')
os.makedirs(QUARANTINE_DIR, exist_ok=True)
from backend.malware_scan import scan_file, clamav_available
...
def _session_timeout_check():
    ...
    if request.endpoint == "static":
        return
    # If user is logged in, enforce idle timeout
    if "username" in session:
        ...
        if last is None:
            ...
            if now_ts - last > app.permanent_session_lifetime.total_seconds():
                session.clear()
...
def inject_session_timeout():
    return {"session_timeout": int(app.permanent_session_lifetime.total_seconds())}
>>>>>>> 013002583c9c6944a86a45dd2040e9c466978da7
# -------------------------
# Routes
# -------------------------
...
"""
                        ...
                        print("⚠ Error sending lock email:", e)
...
def change_password():
    ...
    if request.method == "POST":
        ...
        # Fixed regex: use .* inside lookahead sequences
...
def delete_account():
    ...
    files_col.delete_many({'uploader': u['username']})
...
<<<<<<< HEAD
    ...
    tmpdir = os.path.join(APP_DIR, 'tmp')
    os.makedirs(tmpdir, exist_ok=True)
    temp_path = os.path.join(tmpdir, f"scan_{uuid.uuid4().hex}_{filename}")
    f.save(temp_path)

    # Sanity: 0-byte files are invalid, not "malicious"
    try:
        if os.path.getsize(temp_path) == 0:
            res = {'clean': None, 'error': 'empty_file'}
        else:
            res = scan_file(temp_path, quarantine_dir=QUARANTINE_DIR)
    except Exception as e:
        res = {'clean': None, 'error': f'scan_exception:{e}'}
...
# -------- Uploads with 3-state scan handling --------
=======
    ...
    for f in files:
        if not f or f.filename == '':
            results.append({"filename": None, "clean": None, "error": "empty_upload"})
            continue
        ...
        results.append({"filename": filename, **(res or {})})
...
def files_upload():
    ...
    for f in filesx:
        ...
        temp_fp = os.path.join(tmp_dir, f'tmp_{uuid.uuid4().hex}_{filename}')
        f.save(temp_fp)

        # Quick sanity: reject obviously bad uploads (0 bytes) as invalid, not malicious
        try:
            size_ok = os.path.getsize(temp_fp) > 0
        except Exception:
            size_ok = False

        if not size_ok:
            flash(f'Blocked {filename}: invalid/empty upload')
            logs_col.insert_one({
                'username': u['username'],
                'action': 'blocked_upload',
                'filename': filename,
                'timestamp': now_iso(),
                'details': {'clean': None, 'error': 'empty_file'}
            })
            try:
        ...
        # Run scanner
        try:
            scan_res = scan_file(temp_fp, quarantine_dir=QUARANTINE_DIR)
        except Exception as e:
            scan_res = {'clean': None, 'error': f'scan_exception:{e}'}

        is_clean = scan_res.get('clean')
        signature = scan_res.get('signature')
        scan_error = scan_res.get('error') or scan_res.get('reason')

        if is_clean is False:
            flash(f'Blocked {filename}: {signature or "malicious"}')
            logs_col.insert_one({
                'username': u['username'],
                'action': 'blocked_upload',
                'filename': filename,
        ...
        if is_clean is None or scan_error:
            # Indeterminate scan → allow with status pending_scan (do NOT call it malicious)
            os.replace(temp_fp, dest)
            ...
            logs_col.insert_one({
                'username': u['username'],
                'action': 'upload_pending_scan',
                'filename': filename,
                'timestamp': now_iso(),
                'details': scan_res
            })
            flash(f'Uploaded with pending scan: {filename} ({scan_error or "scan unavailable"})')
...
# Main
# -------------------------
def ai_malicious_check(file_path):
    import subprocess
    ...
    if os.path.getsize(file_path) > max_size:
        is_malicious = True
        details.append("File too large (>250 MB)")
    ...
    try:
        ...
        if "FOUND" in output:
            is_malicious = True
            details.append("ClamAV detected malware")
        else:
            details.append("ClamAV: clean or not available")
    except FileNotFoundError:
        details.append("ClamAV not installed, skipped")
    except Exception as e:
        details.append(f"ClamAV error (ignored): {e}")

    return is_malicious, details

print("✅ ClamAV detected, using industry scanner" if clamav_available() else "⚠ ClamAV not found, using heuristic fallback")
